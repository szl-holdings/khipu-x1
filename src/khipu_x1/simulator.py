...
from .kids import Descriptor, IMPLEMENTED_OPCODES, KhipuValidationError, Opcode, validate_stream
...
class KhipuSimulator:
    ...
    def register_buffer(self, name: str, value: np.ndarray | Iterable[Any]) -> None:
        ...
        if array.dtype.kind in {"f", "c"} and not np.all(np.isfinite(array)):
            raise KhipuExecutionError(f"buffer {name} contains non-finite values")
    ...
    def _execute_one(self, descriptor: Descriptor) -> None:
        ...
        if descriptor.opcode is Opcode.NOP:
            ...
            if len(descriptor.inputs) != 2 or descriptor.output is None:
                raise KhipuExecutionError("GEMM_INT8 requires two inputs and one output")
            ...
            if len(descriptor.inputs) not in {1, 2} or descriptor.output is None:
                raise KhipuExecutionError("RMSNORM requires data, optional weight, and output")
            ...
            denom = np.sqrt(np.mean(np.square(data, dtype=np.float64), axis=-1, keepdims=True) + eps)
            result = data / denom
            ...
            if len(descriptor.inputs) != 1:
                raise KhipuExecutionError("SHA3_COMMIT requires exactly one input")
